...
namespace X_Quartz.Impl
{
	...
	/// jobStore.Initialize();
	/// 
	/// DirectSchedulerFactory.Instance.CreateScheduler("My X_Quartz Scheduler", "My Instance", threadPool, jobStore, "localhost", 1099); 
	/// // don't forget to start the scheduler: 
	/// DirectSchedulerFactory.Instance.GetScheduler("My X_Quartz Scheduler", "My Instance").start();
	/// </pre>
	/// </remarks>>
	...
	public class DirectSchedulerFactory : ISchedulerFactory
	{
		                                    ...
		                                    TimeSpan dbFailureRetryInterval)
		{
			...
			Log.Info(string.Format(CultureInfo.InvariantCulture, "X_Quartz scheduler '{0}", scheduler.SchedulerName));
